/*THE DATASET FOR THIS EXPLORATORY PROJECT WAS MANUALLY INPUTTED BY CREATING THE DIFFERENT TABLES, 
CREDITS TO "Mo Chen" AS THE SOURCE OF THE DATASET VALUES THAT WAS USED TO CREATE ALL TABLES

AT THE END OF THE ENTIRE QUERY, 
YOU CAN FIND THE DATASET VALUES, TABLE FORMULATIONS/MODIFICATIONS FROM LINE 472 DOWNWARDS*/

--The Entire query was done using Microsoft SQL Server

--NOW, THE EXPLORATION BEGINS BELOW:

--To select all records from all the tables
SELECT *
FROM PortCoffeeBiz..Employees

SELECT *
FROM PortCoffeeBiz..Locations

SELECT *
FROM PortCoffeeBiz..Shops

SELECT *
FROM PortCoffeeBiz..Suppliers

--CALCULATING MINIMUM, MAXIMUM AND AVERAGE SALARIES, SEE BELOW:
--To report the maximum salary earned by any employee
SELECT	MAX(Salary) AS Max_Salary
FROM PortCoffeeBiz..Employees

--To report the minimum salary earned by any employee
SELECT	MIN(Salary) AS Min_Salary
FROM PortCoffeeBiz..Employees

--To report the maximum salary earned by a female employee
SELECT	MAX(Salary) AS Max_Salary
FROM PortCoffeeBiz..Employees
WHERE gender = 'F'

--To report the minimum salary earned by a male employee
SELECT	MIN(Salary) AS Min_Salary
FROM PortCoffeeBiz..Employees
WHERE gender = 'M'

--To report the average salary earned by all employees
SELECT	AVG(Salary) AS Average_Salary
FROM PortCoffeeBiz..Employees

--To report the average salary earned by all male employees
SELECT	AVG(Salary) AS Average_Salary
FROM PortCoffeeBiz..Employees
WHERE gender = 'M'
...
--TO COUNT THE NUMBER OF SALARY-FILLED COLUMNS/ENTRIES
SELECT COUNT(Salary)
FROM PortCoffeeBiz..Employees


--TO COUNT THE NUMBER OF EMAIL-FILLED ROWS/ENTRIES
SELECT COUNT(email)
FROM PortCoffeeBiz..Employees

--TO CALCULATE THE AVERAGE SALARIES OF ALL MALE EMPLOYEES ACCROSS THE DIFFERENT COFFEE SHOPS
...
--TO CALCULATE THE AVERAGE SALARIES OF ALL FEMALE EMPLOYEES ACCROSS DIFFERENT COFFEE SHOPS
SELECT	coffeeshop_id, 
		AVG(Salary) as Avg_Salary, 
		gender
FROM	PortCoffeeBiz..Employees
...
--TO CALCULATE THE AVERAGE SALARIES OF BOTH MALE AND FEMALE EMPLOYEES SHOWING GENDERS ACCROSS DIFFERENT COFFEE SHOPS (AN OVERVIEW OF THE 2 QUERIES ABOVE)
SELECT	coffeeshop_id, 
		AVG(Salary) as Avg_Salary, 
		gender
FROM	PortCoffeeBiz..Employees
...
--TO CALCULATE THE AVERAGE SALARIES OF BOTH MALE AND FEMALE ACCROSS DIFFERENT COFFEE SHOPS
SELECT	coffeeshop_id, 
		AVG(Salary) as Avg_Salary--, gender --(Removing the comment here will report the Avg salary by the Male and Female separately)
FROM	PortCoffeeBiz..Employees
GROUP BY coffeeshop_id--, gender
ORDER BY 2 DESC

--TO SEE THE NUMBER OF EMPLOYEES IN EACH COFFEE SHOP
SELECT	coffeeshop_id,
...
--TO SEE THE NUMBER OF EMPLOYEES IN EACH COFFEE SHOP BY THE COFFEE SHOP ID AND NAME
SELECT	a.coffeeshop_id, 
		b.coffeeshop_name,
		COUNT(a.employee_id) as Employee_count
FROM	PortCoffeeBiz..Employees a
		JOIN PortCoffeeBiz..Shops b
			ON a.coffeeshop_id=b.coffeeshop_id
GROUP BY a.coffeeshop_id, 
		b.coffeeshop_name
ORDER BY 3 DESC

--TO SEE THE NUMBER OF EMPLOYEES IN EACH COFFEE SHOP BY THE COFFEE SHOP ID, COFFEE SHOP NAME AND CITY
SELECT	a.coffeeshop_id, 
		b.coffeeshop_name, 
		c.city,
		COUNT(a.employee_id) as Employee_count
FROM	(PortCoffeeBiz..Employees a
		JOIN PortCoffeeBiz..Shops b
			ON a.coffeeshop_id=b.coffeeshop_id)
				JOIN PortCoffeeBiz..Locations c
...
-- TO view the summary of the salary aggregations and employee count by the coffeeshop ID
SELECT	coffeeshop_id, 
...
-- TO view the summary of the salary aggregations and employee count of employees (earning at least $10,200) by the coffeeshop ID and coffeeshop name
SELECT	a.coffeeshop_id, 
		b.coffeeshop_name,
		COUNT(*) AS number_of_employees,
...
GROUP BY a.coffeeshop_id, 
		b.coffeeshop_name
HAVING	MIN(a.salary) >= 10200 --(HAVING clause is like a WHERE statement (to filter out) that can ONLY be used after a GROUP BY statement)
ORDER BY 2 DESC

-- TO SEE ALL EMPLOYEES EARNING SALARIES HIGHER THAN $50,000
SELECT	employee_id, 
		first_name, 
		last_name, 
		hire_date
FROM	PortCoffeeBiz..Employees
WHERE	Salary > 50000
			--AND gender='F' --(If commented out, this will return only the Female employees in this category)
...
--TO SEE ALL ENTRY HAVING EMAIL REGISTERED INTO THE DATABASE
SELECT * --employee_id, first_name, last_name, hire_date
FROM PortCoffeeBiz..Employees
WHERE email IS NOT NULL
--WHERE NOT email IS NULL

--TO SEE THE TOP 15 EMPLOYEES EARNING SALARY BETWEEN 48000 AND 65000
SELECT TOP 15 
		employee_id, 
		first_name, 
		last_name, 
...
WHERE	Salary BETWEEN 48000 AND 65000
			--AND gender = 'F' --(If commented out, this will return only the top 15 Female employees in this category)
ORDER BY salary DESC

--TO SEE THE DIFFERENT COFFEESHOP ID REGISTERED INTO THE DATABASE
SELECT DISTINCT coffeeshop_id
FROM PortCoffeeBiz..Employees

--TO SEE THE DIFFERENT EMPLOYEE GENDERS REGISTERED IN THE DATABASE
SELECT DISTINCT gender
FROM PortCoffeeBiz..Employees

--To count the different gender of employee
SELECT	gender, 
...
--TO SEE THE DIFFERENT COUNTRIES REGISTERED INTO THE DATABASE (WHERE THE COFFEE SHOP EXISTS)
SELECT DISTINCT country
FROM PortCoffeeBiz..Locations

--TO SPLIT THE FULL DATE INTO SEPARATE PARTS AS YEAR, MONTH AND DAY
SELECT hire_date as Full_date,
       YEAR(hire_date) AS Year,
       MONTH(hire_date) AS Month,
       DAY(hire_date) AS Day
FROM PortCoffeeBiz..Employees

--TO CHECK THE EMAIL LENGTH OF EMPLOYEES THAT REGISTERED THEIR EMAILS INTO THE DATABASE
SELECT	first_name, 
		last_name, 
		email, 
...
--EXECUTING CONCATENATE STATEMENT TO CREATE SENTENCES FROM THE DIFFERENT COLUMN (attribute) OF THE TABLE
SELECT --TOP 10
	CONCAT(first_name, ' ', last_name, ' earns ', '$', Salary, '.') AS What_Employee_earns,
	Salary
FROM PortCoffeeBiz..Employees 
ORDER BY Salary DESC

SELECT --TOP 10
		CONCAT(first_name, ' ', last_name, ' hired on ', hire_date, ' earns ', '$', Salary) AS What_Employee_earns,
		Salary, 
		gender
FROM PortCoffeeBiz..Employees 
ORDER BY Salary DESC
...
--CASE STATEMENT
--To formulate category by employee's salary
SELECT CONCAT(first_name, ' ', last_name) AS Fullname,
		CASE 
			WHEN salary BETWEEN 0 AND 50000 THEN 'Below_50k'
			ELSE 'Above_50k'
		END
			AS Salary_Status
FROM PortCoffeeBiz..Employees 

--MORE ON CONCATENATE AND CASE STATEMENTS COMBINED 
--To formulate "True/False" column for Male employees earning Below 50000
SELECT CONCAT(first_name, ' ', last_name) AS Fullname, 
		Salary, 
		Gender,
		CASE 
			WHEN salary < 50000 AND gender= 'M' THEN 'True'
			ELSE 'False'
		END AS Below_50k_Male
FROM PortCoffeeBiz..Employees

--To report ONLY the "True" column for above query
--To see only the male employees that earn salaries below $50,000
SELECT CONCAT(first_name, ' ', last_name) AS Fullname, 
       Salary, 
       Gender,
       CASE 
...
--To report ONLY the FALSE column for above query
--To see either gender of the employees that earn salaries above $50,000
SELECT CONCAT(first_name, ' ', last_name) AS Fullname, 
       Salary, 
       Gender,
       CASE 
       ...
       END AS Below_50k_Male
FROM PortCoffeeBiz..Employees
WHERE NOT (salary < 50000 AND gender= 'M')
ORDER BY 2 DESC


--MORE ON CASE AND CONCATENATE STATEMENTS COMBINED
SELECT	CONCAT(first_name, ' ', last_name, ' earns ', 
		(CASE 
			WHEN salary BETWEEN 0 AND 50000 THEN 'Below_50k'
			ELSE 'Above_50k'
		END)) AS Range_of_Employee_Earnings,
		Salary
FROM PortCoffeeBiz..Employees 
--WHERE salary>50000 (This will report all employees earning Salary Above $50,000)
ORDER BY 2 DESC

--To see employees having email domain '%@yahoo.com' OR '%@gmail.com'
SELECT	CONCAT(first_name, ' ', last_name) AS Fullname, 
		Gender, 
		email
FROM	PortCoffeeBiz..Employees
...
--To see only male employees having email domain '%@yahoo.com' OR '%@gmail.com' (SHOULD REPORT NONE)
SELECT	CONCAT(first_name, ' ', last_name) AS Fullname, 
		Gender, 
		email
FROM	PortCoffeeBiz..Employees
...
--SUBSTRING FUNCTION
SELECT	email, 
		SUBSTRING(email, 7, 30) AS trucated_email
FROM	PortCoffeeBiz..Employees

SELECT	SUBSTRING(CONCAT(first_name, ' ', last_name),1, 9) AS Trunc_name, 
		Gender, 
		email
FROM	PortCoffeeBiz..Employees

--TO FIND POSITION OF CHARACTER "@" IN THE EMAIL ADDRESS OF EMPLOYEES
SELECT	email,
		CHARINDEX('@', email) AS '@_Position'
FROM	PortCoffeeBiz..Employees
WHERE	email IS NOT NULL

--SUBSTRING FUNCTION
--To see the email domain of employees (different scenarios are given here with others commented out and only 1 allowed to report per time)
SELECT	CONCAT(first_name, ' ', last_name) AS Fullname,
		SUBSTRING(email, CHARINDEX('@', email) + 1, LEN(email)) AS Email_domain, 
		--SUBSTRING(email, CHARINDEX('@', email), LEN(email)) AS Email_domain, --(Domain Starting from "@")
		--SUBSTRING(email, 1, CHARINDEX('@', email)-1) AS UserName, --(Reports employee's username before the email domain)
		Gender, 
		email
FROM	PortCoffeeBiz..Employees
WHERE	email IS NOT NULL

--To count the different email domains of employees
SELECT	SUBSTRING(email, CHARINDEX('@', email), LEN(email)) AS Email_domain,
		COUNT (SUBSTRING(email, CHARINDEX('@', email), LEN(email))) AS Email_Domain_Count
FROM	PortCoffeeBiz..Employees
WHERE	email IS NOT NULL
--AND email LIKE '%.gov' --(comment this out to see the number of employees whose email end with ".gov")
GROUP BY SUBSTRING(email, CHARINDEX('@', email), LEN(email))
ORDER BY 2 DESC

-- COALESCE 
-- To fill missing emails with custom value 'EMAIL NOT PROVIDED'
SELECT	email, 
		COALESCE(email, 'EMAIL NOT PROVIDED')
FROM	PortCoffeeBiz..Employees
--WHERE email IS NULL --(this would report only the null email columns and the modification made)

--To report the employee count in each salary category formulated by the CASE statement in the query
SELECT	a.Salary_category,
		COUNT(*) AS SalaryCategoryCount
...
-- To view the difference between an employee salary with the general average salary
SELECT	CONCAT(first_name, ' ', last_name) AS Fullname,
		salary, 
		(salary - (SELECT AVG(salary) FROM PortCoffeeBiz..Employees)) AS AverageSalaryDiff
FROM	PortCoffeeBiz..Employees
ORDER BY 3 DESC

-- To view the nature of difference between an employee salary with the general average salary
SELECT	CONCAT(first_name, ' ', last_name) AS Fullname,
		salary, 
		(salary - (SELECT AVG(salary)FROM PortCoffeeBiz..Employees)) AS AverageSalaryDiff,
		CASE
			WHEN (salary - (SELECT AVG(salary) FROM PortCoffeeBiz..Employees)) < 0 THEN 'Negative Diff'
			ELSE 'Positive Diff'
		END AS Nature_of_Sal_Diff
FROM	PortCoffeeBiz..Employees
--ORDER BY 3 DESC --(If this is commented out, obviously the positive values will come first before the negative ones)

--To see all coffee shops located in the US,
--This query will be done in 2 ways: (1) Using SUBQUERY and, (2) Using JOIN statement

--(1) USING SUBQUERY (Where in Where Subquery)
SELECT * 
FROM PortCoffeeBiz..shops
...
--(2) USING JOIN statement (JOIN which is thesame as INNER JOIN)
SELECT	a.coffeeshop_id, 
		a.coffeeshop_name, 
		b.city_id
FROM	PortCoffeeBiz..shops a
		JOIN PortCoffeeBiz..locations b
			ON a.city_id=b.city_id
WHERE	country = 'United States'

-- To see all employees who work in US coffee shops
--This query was done in 2 ways: (1) Using SUBQUERY and, (2) Using JOIN statement

--(1) USING SUBQUERY (Where in Where Subquery)
SELECT *
FROM PortCoffeeBiz..employees
WHERE /*salary>35000 AND*/ coffeeshop_id IN --IF THE COMMENTED PORTION IS MADE ACTIVE, it will report all employees earning over $35,000 working in the US coffee shop
	(SELECT coffeeshop_id 
		FROM PortCoffeeBiz..shops
		WHERE city_id IN 
			(SELECT city_id 
...
SELECT	a.employee_id, 
		...
		a.coffeeshop_id 
FROM	PortCoffeeBiz..Employees a
		JOIN PortCoffeeBiz..shops b ON a.coffeeshop_id = b.coffeeshop_id
		JOIN PortCoffeeBiz..locations c ON b.city_id = c.city_id
WHERE	c.country = 'United States' --AND a.salary>35000 --IF THE COMMENTED PORTION IS MADE ACTIVE, it will report all employees earning over $35,000

--CHECKING OUT TRANSPOSE FUNCTION, WHICH IS INTERCHANGING THE ROWS WITH THE COLUMNS
--transposing table of the employee count filtered by the pay category
SELECT
	SUM(CASE 
...
--JUST MESSING AROUND WITH LENGTH CLAUSE
SELECT	LEN('     HELLO     ') AS hello_with_spaces,
		LEN('HELLO') AS hello_no_spaces,
		LEN(TRIM('     HELLO     ')) AS hello_trimmed; --Showing the effect of Trimming which is a Data Cleaning Process

SELECT 'COME' as come_here


/*=========SEE THE DATASET BELOW FOR THE ENTIRE QUERY WRITTEN ABOVE============
EVERYTHING HERE HAS BEEN COMMENTED OUT

--CREATING ALL THE TABLES, SETTING THE PRIMARY AND FOREIGN KEYS
...
CREATE TABLE Employees
    ...
    email VARCHAR(50),
    hire_date DATE,
    gender VARCHAR(1), -- Male or Female represented as M or F
    salary INT,
    coffeeshop_id INT) -- This will be a Foreign key to link other tables (Shops table)

-- To Create the Shops table
CREATE TABLE Shops 
	(coffeeshop_id INT PRIMARY KEY,
    coffeeshop_name VARCHAR(50),
    city_id INT) -- This will be a Foreign key to link other tables (Locations table)

-- To Add foreign key to the Employees table
...
-- To Add foreign key to shops table
ALTER TABLE Shops
...
CREATE TABLE Suppliers
    ...
    PRIMARY KEY (coffeeshop_id, supplier_name), --2 Pry keys selected
    FOREIGN KEY (coffeeshop_id) 
	REFERENCES shops(coffeeshop_id)
    ON DELETE CASCADE)

--=====BELOW SHOWS THE INSERTING OF VALUES INTO ALL THE TABLES CREATED=====
...
--INSERTING THE REMAINING VALUES INTO THE TABLES:::
--SEE BELOW::
...
SELECT	e1.hire_date,
		...
		FROM PortCoffeeBiz..Employees e2
		WHERE e2.hire_date BETWEEN DATEADD(day, -30, e1.hire_date) 
			AND e1.hire_date) AS pay_pattern
FROM	PortCoffeeBiz..Employees e1
ORDER BY 1